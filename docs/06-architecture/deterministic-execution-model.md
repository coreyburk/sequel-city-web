...
- suspect verification verdicts are generated by SQL Server through the `Solution` trigger
...
The dedicated suspect verification endpoint is a backend-owned database-backed exception for the original assignment's solution flow. It does not change learner SQL editor rules, and general `INSERT` statements remain blocked through `POST /api/query/execute`.

## Suspect Verification Model

When a suspect is submitted to `POST /api/case/verify-suspect`:

1. the route validates that the request body contains a non-empty string `suspect` field
2. the backend service trims the suspect name
3. the backend inserts the suspect into `Solution` using parameterized SQL
4. SQL Server trigger behavior generates the verdict text
5. the backend reads the latest non-null verdict for that suspect
6. the backend returns the suspect and verdict without hard-coding hidden suspect identities in application code
