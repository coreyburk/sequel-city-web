# Gemini Copy-Paste Scene Prompts

## Purpose

Use these self-contained prompts directly in Gemini / Nano Banana to generate Student Mode scene artwork. Each prompt includes the shared visual direction, scene-specific requirements, composition constraints, and negative prompt so it can be pasted independently.

Generate at least four candidates per scene. Select only images that remain readable in a narrow web-app header crop, contain no readable text, preserve useful case-file texture, and feel like part of the same detective case world. Blurred handwriting-like marks, ruled ledger rows, indistinct form fields, and out-of-focus photo evidence are allowed and encouraged; legible words, dates, names, SQL, and spoiler facts are not.

## Prompt 1: Crime Ledger
