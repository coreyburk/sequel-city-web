...
The current runtime has case-id keyed local browser persistence for learner-owned frontend progress. Released cases `case-001` and `case-004` each own their storage contracts; locked and future case ids must not hydrate or write investigation progress. The student workspace stores convenience state in browser `localStorage`, including notebook entries, pinned fact page placement, current student view, draft query text, Samuel step, visible milestone state, pending evidence step, earned check-in IDs, and visible suspect-theory display state.

The local storage envelope is versioned and includes the active case id plus the persisted frontend state payload. Case 004 validates its own milestones, views, pending evidence steps, and suspect-verification response shape. Case 001 uses `sequel-city.case-001.student-state.v1` for learner-owned view, draft, manual notes, and M1/M2 query references; it ignores stored completion flags and re-executes saved references through the API before restoring visible milestones. Future playable cases must define their own storage validation contract before their progress can be restored.
...
Malformed, unsupported-version, wrong-case, locked-case, or future-case browser storage is ignored and the app falls back to authored defaults. Both released cases expose user-confirmed reset/clear-progress behavior with case-specific isolation. Case 001 clears only `sequel-city.case-001.student-state.v1` and resets its in-memory M1/M2 state; it does not access Case 004 thread storage. Case 004 clears only its learner-owned progress and thread keys. Neither reset clears backend query history, database state, account/cloud data, browser history state, case-library metadata, locked/future case data, or unrelated localStorage keys.

### Playable Case Module Boundary

A case becomes playable only when it has an explicit playable-case module contract. The current released implementation registers `case-001` and `case-004` as normal playable cases. App entry and investigation rendering consume this playable-case module registry; case-library `isUnlocked` metadata alone is not sufficient to enter, restore, or render an investigation. Locked and future case library entries are presentation metadata only until a scoped work package defines their playable module.
...
Case 001 (`The Clocktower Poisoning`) is a normal Foundations release with the same student shell structure as Case 004: Samuel's Briefing, Query Lab, Query Runner, Query Results, Case File, and Evidence Board. Its runtime scope is exactly two public SQL milestones: `case-001-clocktower-report-located` and `case-001-report-interviews-located`. Query execution transports Case 001 metadata only on successful, explicit `case-001` M1/M2 requests. The current request shape retains `isSkeletonGateEnabled: true` as a legacy transport compatibility field; the released client supplies it and normal entry does not require `VITE_ENABLE_CASE_001_PLAYABLE_SKELETON`. The shell renders learner-visible result rows and non-spoiler feedback, retains learner-owned state locally, and revalidates saved M1/M2 query references through the API before restoring visible milestones. It does not create durable Case 001 investigation threads, verify suspects, expose answer keys, add runtime AI, or treat frontend state as authoritative milestone progression. Metadata is absent for no-opt-in requests, wrong case ids, wrong milestone ids, blocked SQL, restricted-table SQL, malformed requests, and execution failures.
...
Persistence infrastructure must not be invented speculatively. Any future backend, account, cloud, cross-device, or multi-user persistence layer must be introduced through a scoped work package that defines the storage approach, authority boundaries, and spoiler-safe recovery behavior.
