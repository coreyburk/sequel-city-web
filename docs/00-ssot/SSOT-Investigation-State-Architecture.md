...
Case 001 (`The Clocktower Poisoning`) has a dev/test-only shared playable shell preview behind the explicit `VITE_ENABLE_CASE_001_PLAYABLE_SKELETON === "true"` gate. With the gate disabled, Case 001 remains locked, its landing action remains `Archive Locked`, browser-history restoration cannot render its investigation view, and no Case 001 progress storage is written. With the gate enabled, Case 001 enters the same student shell structure as Case 004: Samuel's Briefing, Query Lab, Query Runner, Query Results, Case File, and Evidence Board. Its current runtime scope is limited to the M1-M3 public clocktower SQL path: `case-001-clocktower-report-located`, `case-001-report-interviews-located`, and `case-001-witness-identities-resolved`. Query execution can transport Case 001 milestone metadata only on successful responses when the request explicitly opts into `case-001`, one supported M1-M3 milestone id, and an enabled skeleton gate. The shared shell may render learner-visible result rows and non-spoiler milestone feedback, and it may keep component-memory-only M1-M3 display progress and notebook notes for the active page session. It must not write Case 001 localStorage, release the case, persist progress, create durable investigation threads, verify suspects, expose answer keys, add runtime AI, or treat frontend state as authoritative milestone progression. The transport metadata is absent for no-opt-in requests, disabled gate input, wrong case ids, wrong milestone ids, blocked SQL, restricted-table SQL, malformed requests, and execution failures.
