...
describe("App", () => {
  ...
  it("defaults to student mode with minimal story, schema snapshot, and query lab", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Case 004 · SELECT * FROM Suspects · 0/6 clues logged" })
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole("button", { name: "Talk to Samuel" })).toHaveAttribute(
      "aria-pressed",
      "true"
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: "Open Evidence Board" }));
  ...
  it("advances Samuel Tupleton's briefing through the opening breadcrumbs", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Talk to Samuel" }));
    expect(
