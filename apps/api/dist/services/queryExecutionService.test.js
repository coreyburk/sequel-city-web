...
const testCases = [
    {
        ...
        run: async () => {
            const queryExecutionService = require("./queryExecutionService.ts");
    ...
    },
    {
        ...
        run: async () => {
            const queryExecutionService = require("./queryExecutionService.ts");
            const result = await queryExecutionService.executeSafeQuery("DELETE FROM PersonsOfInterest");
            assert.equal(result.success, false);
            assert.equal(result.safety.isAllowed, false);
    ...
    },
    {
        ...
        run: async () => {
            ...
            const result = await queryExecutionService.executeSafeQuery("SELECT * FROM dbo.Solution", async () => {
                executorCallCount += 1;
                return [];
            });
            const history = queryHistoryService.getQueryHistoryRecords();
    ...
    },
    {
        ...
        run: async () => {
            ...
            for (const sql of blockedQueries) {
                let executorCallCount = 0;
                const result = await queryExecutionService.executeSafeQuery(sql, async () => {
                    executorCallCount += 1;
                    return [];
                });
                assert.equal(executorCallCount, 0);
    ...
    },
    {
        ...
        run: async () => {
            ...
            const result = await queryExecutionService.executeSafeQuery("SELECT suspectName, clueCount, isSolved, occurredAt, notes, payload FROM CaseFiles", async () => recordset);
    ...
    },
    {
        ...
        run: async () => {
            ...
            const queryHistoryService = require("./queryHistoryService.ts");
            queryHistoryService.resetQueryHistoryForTests();
            const result = await queryExecutionService.executeSafeQuery("SELECT 1", async () => {
            ...
            assert.equal("columns" in result, false);
            assert.equal("rowCount" in result, false);
            assert.equal(result.message, "Query execution failed. Verify the SQL and database connection.");
            const history = queryHistoryService.getQueryHistoryRecords();
            assert.equal(history.length, 1);
...
async function runTests() {
    ...
    if (failedCount > 0) {
        ...
        console.log("NOTE Safe SELECT execution is not covered here; it requires integration testing against a local SQL Server instance.");
    }
}
